# Organizations

## Model definitions

### Organization

Organization

| Name | abbreviation | name | registry | registry_identifier |
|------|------|------|------|------|
| `AA_OPTO_ELECTRONIC` | `None` | `AA Opto Electronic` | `None` | `None` |
| `ABCAM` | `None` | `Abcam` | `name='Research Organization Registry' abbreviation='ROR'` | `02e1wjw63` |
| `ADDGENE` | `None` | `Addgene` | `name='Research Organization Registry' abbreviation='ROR'` | `01nn1pw54` |
| `AI` | `AI` | `Allen Institute` | `name='Research Organization Registry' abbreviation='ROR'` | `03cpe7c52` |
| `AIBS` | `AIBS` | `Allen Institute for Brain Science` | `name='Research Organization Registry' abbreviation='ROR'` | `00dcv1019` |
| `AILIPU` | `Ailipu` | `Ailipu Technology Co` | `None` | `None` |
| `AIND` | `AIND` | `Allen Institute for Neural Dynamics` | `name='Research Organization Registry' abbreviation='ROR'` | `04szwah67` |
| `AMS_OSRAM` | `None` | `ams OSRAM` | `name='Research Organization Registry' abbreviation='ROR'` | `045d0h266` |
| `ARECONT_VISION_COSTAR` | `None` | `Arecont Vision Costar` | `None` | `None` |
| `ASI` | `ASI` | `Applied Scientific Instrumentation` | `None` | `None` |
| `ASUS` | `None` | `ASUS` | `name='Research Organization Registry' abbreviation='ROR'` | `00bxkz165` |
| `BASLER` | `None` | `Basler` | `None` | `None` |
| `BCM` | `BCM` | `Baylor College of Medicine` | `name='Research Organization Registry' abbreviation='ROR'` | `02pttbw34` |
| `BU` | `BU` | `Boston University` | `name='Research Organization Registry' abbreviation='ROR'` | `05qwgg493` |
| `CAMBRIDGE_TECHNOLOGY` | `None` | `Cambridge Technology` | `None` | `None` |
| `CARL_ZEISS` | `None` | `Carl Zeiss` | `name='Research Organization Registry' abbreviation='ROR'` | `01xk5xs43` |
| `CATHETER_IMPLANT_INSTITUTIONS` | `N/A` | `N/A` | `N/A` | `N/A` |
| `CHAMPALIMAUD` | `Champalimaud` | `Champalimaud Foundation` | `name='Research Organization Registry' abbreviation='ROR'` | `03g001n57` |
| `CHROMA` | `None` | `Chroma` | `None` | `None` |
| `COHERENT_SCIENTIFIC` | `None` | `Coherent Scientific` | `name='Research Organization Registry' abbreviation='ROR'` | `031tysd23` |
| `COLUMBIA` | `Columbia` | `Columbia University` | `name='Research Organization Registry' abbreviation='ROR'` | `00hj8s172` |
| `COMPUTAR` | `None` | `Computar` | `None` | `None` |
| `CONOPTICS` | `None` | `Conoptics` | `None` | `None` |
| `CRL` | `CRL` | `Charles River Laboratories` | `name='Research Organization Registry' abbreviation='ROR'` | `03ndmsg87` |
| `CUSTOM` | `None` | `Custom` | `None` | `None` |
| `CZI` | `CZI` | `Chan Zuckerberg Initiative` | `name='Research Organization Registry' abbreviation='ROR'` | `02qenvm24` |
| `DAQ_DEVICE_MANUFACTURERS` | `N/A` | `N/A` | `N/A` | `N/A` |
| `DETECTOR_MANUFACTURERS` | `N/A` | `N/A` | `N/A` | `N/A` |
| `DIGIKEY` | `None` | `DigiKey` | `None` | `None` |
| `DODOTRONIC` | `None` | `Dodotronic` | `None` | `None` |
| `DORIC` | `None` | `Doric` | `name='Research Organization Registry' abbreviation='ROR'` | `059n53q30` |
| `EALING` | `None` | `Ealing` | `None` | `None` |
| `EDMUND_OPTICS` | `None` | `Edmund Optics` | `name='Research Organization Registry' abbreviation='ROR'` | `01j1gwp17` |
| `EMORY` | `Emory` | `Emory University` | `name='Research Organization Registry' abbreviation='ROR'` | `03czfpz43` |
| `EURESYS` | `None` | `Euresys` | `None` | `None` |
| `FILTER_MANUFACTURERS` | `N/A` | `N/A` | `N/A` | `N/A` |
| `FLIR` | `FLIR` | `Teledyne FLIR` | `name='Research Organization Registry' abbreviation='ROR'` | `01j1gwp17` |
| `FUJINON` | `None` | `Fujinon` | `None` | `None` |
| `FUNDERS` | `N/A` | `N/A` | `N/A` | `N/A` |
| `HAMAMATSU` | `None` | `Hamamatsu` | `name='Research Organization Registry' abbreviation='ROR'` | `03natb733` |
| `HAMILTON` | `None` | `Hamilton` | `None` | `None` |
| `HUST` | `HUST` | `Huazhong University of Science and Technology` | `name='Research Organization Registry' abbreviation='ROR'` | `00p991c53` |
| `IDT` | `IDT` | `Integrated DNA Technologies` | `name='Research Organization Registry' abbreviation='ROR'` | `009jvpf03` |
| `IMEC` | `IMEC` | `Interuniversity Microelectronics Center` | `name='Research Organization Registry' abbreviation='ROR'` | `02kcbn207` |
| `INFINITY_PHOTO_OPTICAL` | `None` | `Infinity Photo-Optical` | `None` | `None` |
| `INVITROGEN` | `None` | `Invitrogen` | `name='Research Organization Registry' abbreviation='ROR'` | `03x1ewr52` |
| `IR_ROBOT_CO` | `None` | `IR Robot Co` | `None` | `None` |
| `ISL` | `ISL` | `ISL Products International` | `None` | `None` |
| `ITEM` | `None` | `Item` | `None` | `None` |
| `JANELIA` | `Janelia` | `Janelia Research Campus` | `name='Research Organization Registry' abbreviation='ROR'` | `013sk6x84` |
| `JAX` | `JAX` | `Jackson Laboratory` | `name='Research Organization Registry' abbreviation='ROR'` | `021sy4w91` |
| `JHU` | `JHU` | `Johns Hopkins University` | `name='Research Organization Registry' abbreviation='ROR'` | `00za53h95` |
| `JULABO` | `None` | `Julabo` | `None` | `None` |
| `KOWA` | `None` | `Kowa` | `name='Research Organization Registry' abbreviation='ROR'` | `03zbwg482` |
| `LASER_MANUFACTURERS` | `N/A` | `N/A` | `N/A` | `N/A` |
| `LED_MANUFACTURERS` | `N/A` | `N/A` | `N/A` | `N/A` |
| `LEICA` | `None` | `Leica` | `None` | `None` |
| `LENS_MANUFACTURERS` | `N/A` | `N/A` | `N/A` | `N/A` |
| `LG` | `None` | `LG` | `name='Research Organization Registry' abbreviation='ROR'` | `02b948n83` |
| `LIFECANVAS` | `None` | `LifeCanvas` | `None` | `None` |
| `LUMEN_DYNAMICS` | `None` | `Lumen Dynamics` | `None` | `None` |
| `MANIPULATOR_MANUFACTURERS` | `N/A` | `N/A` | `N/A` | `N/A` |
| `MBF` | `MBF` | `MBF Bioscience` | `name='Research Organization Registry' abbreviation='ROR'` | `02zynam48` |
| `MEADOWLARK_OPTICS` | `None` | `Meadowlark Optics` | `name='Research Organization Registry' abbreviation='ROR'` | `00n8qbq54` |
| `MIBR` | `MIBR` | `McGovern Institute for Brain Research` | `name='Research Organization Registry' abbreviation='ROR'` | `05ymca674` |
| `MIDOPT` | `MidOpt` | `Midwest Optical Systems, Inc.` | `None` | `None` |
| `MIT` | `MIT` | `Massachusetts Institute of Technology` | `name='Research Organization Registry' abbreviation='ROR'` | `042nb2s44` |
| `MITUTUYO` | `None` | `Mitutuyo` | `None` | `None` |
| `MIT_BCS` | `MIT-BCS` | `MIT Department of Brain and Cognitive Sciences` | `None` | `None` |
| `MJFF` | `MJFF` | `Michael J. Fox Foundation for Parkinson's Research` | `name='Research Organization Registry' abbreviation='ROR'` | `03arq3225` |
| `MKS_NEWPORT` | `None` | `MKS Newport` | `name='Research Organization Registry' abbreviation='ROR'` | `00k17f049` |
| `MONITOR_MANUFACTURERS` | `N/A` | `N/A` | `N/A` | `N/A` |
| `MPI` | `MPI` | `MPI` | `None` | `None` |
| `NATIONAL_INSTRUMENTS` | `None` | `National Instruments` | `name='Research Organization Registry' abbreviation='ROR'` | `026exqw73` |
| `NAVITAR` | `None` | `Navitar` | `None` | `None` |
| `NCCIH` | `NCCIH` | `National Center for Complementary and Integrative Health` | `name='Research Organization Registry' abbreviation='ROR'` | `00190t495` |
| `NEUROPHOTOMETRICS` | `None` | `Neurophotometrics` | `None` | `None` |
| `NEW_SCALE_TECHNOLOGIES` | `None` | `New Scale Technologies` | `None` | `None` |
| `NIKON` | `None` | `Nikon` | `name='Research Organization Registry' abbreviation='ROR'` | `0280y9h11` |
| `NIMH` | `NIMH` | `National Institute of Mental Health` | `name='Research Organization Registry' abbreviation='ROR'` | `04xeg9z08` |
| `NINDS` | `NINDS` | `National Institute of Neurological Disorders and Stroke` | `name='Research Organization Registry' abbreviation='ROR'` | `01s5ya894` |
| `NRESEARCH_INC` | `None` | `NResearch Inc` | `None` | `None` |
| `NYU` | `NYU` | `New York University` | `name='Research Organization Registry' abbreviation='ROR'` | `0190ak572` |
| `OEPS` | `OEPS` | `Open Ephys Production Site` | `name='Research Organization Registry' abbreviation='ROR'` | `007rkz355` |
| `OLYMPUS` | `None` | `Olympus` | `name='Research Organization Registry' abbreviation='ROR'` | `02vcdte90` |
| `OPTOTUNE` | `None` | `Optotune` | `None` | `None` |
| `OTHER` | `None` | `Other` | `None` | `None` |
| `OXXIUS` | `None` | `Oxxius` | `None` | `None` |
| `PLACID_INDUSTRIES` | `None` | `Placid Industries` | `None` | `None` |
| `PRIZMATIX` | `None` | `Prizmatix` | `None` | `None` |
| `QUANTIFI` | `None` | `Quantifi` | `None` | `None` |
| `RASPBERRY_PI` | `None` | `Raspberry Pi` | `None` | `None` |
| `RESEARCH_INSTITUTIONS` | `N/A` | `N/A` | `N/A` | `N/A` |
| `SAME_SKY` | `None` | `Same Sky` | `None` | `None` |
| `SCHNEIDER_KREUZNACH` | `None` | `Schneider-Kreuznach` | `None` | `None` |
| `SECOND_ORDER_EFFECTS` | `None` | `Second Order Effects` | `None` | `None` |
| `SEMROCK` | `None` | `Semrock` | `None` | `None` |
| `SICGEN` | `None` | `SICGEN` | `None` | `None` |
| `SIGMA_ALDRICH` | `None` | `Sigma-Aldrich` | `None` | `None` |
| `SIMONS_FOUNDATION` | `None` | `Simons Foundation` | `name='Research Organization Registry' abbreviation='ROR'` | `01cmst727` |
| `SPEAKER_MANUFACTURERS` | `N/A` | `N/A` | `N/A` | `N/A` |
| `SPINNAKER` | `None` | `Spinnaker` | `None` | `None` |
| `SUBJECT_SOURCES` | `N/A` | `N/A` | `N/A` | `N/A` |
| `TAMRON` | `None` | `Tamron` | `None` | `None` |
| `THERMO_FISHER_SCIENTIFIC` | `None` | `Thermo Fisher Scientific` | `name='Research Organization Registry' abbreviation='ROR'` | `03x1ewr52` |
| `THE_IMAGING_SOURCE` | `None` | `The Imaging Source` | `None` | `None` |
| `THE_LEE_COMPANY` | `None` | `The Lee Company` | `None` | `None` |
| `THORLABS` | `None` | `Thorlabs` | `name='Research Organization Registry' abbreviation='ROR'` | `04gsnvb07` |
| `TMC` | `TMC` | `Technical Manufacturing Corporation` | `None` | `None` |
| `TRANSDUCER_TECHNIQUES` | `None` | `Transducer Techniques` | `None` | `None` |
| `TWCF` | `TWCF` | `Templeton World Charity Foundation` | `name='Research Organization Registry' abbreviation='ROR'` | `00x0z1472` |
| `TYMPHANY` | `None` | `Tymphany` | `None` | `None` |
| `UCSD` | `UCSD` | `University of California, San Diego` | `name='Research Organization Registry' abbreviation='ROR'` | `0168r3w48` |
| `VIEWORKS` | `None` | `Vieworks` | `None` | `None` |
| `VORTRAN` | `None` | `Vortran` | `None` | `None` |


