...
| `funding_source` | List[{Funding}] | Funding source. If internal funding, select 'Allen Institute' |
...
| `investigators` | List[{Person}] | Full name(s) of key investigators (e.g. PI, lead scientist, contact person) |
...
| `fundee` | Optional[{Person}] | Person(s) funded by this mechanism |
