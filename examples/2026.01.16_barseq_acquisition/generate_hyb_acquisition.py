"""Generate BARseq Hybridization acquisition metadata."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Generate BARseq hybridization acquisition metadata JSON."
    )
    parser.add_argument(
        "--subject-id",
        default="780346",
...
def main():
    """Generate hybridization acquisition metadata."""
    args = parse_args()
    ...
    output_file = Path(__file__).parent / f"barseq_{subject_id}_hyb_acquisition.json"
    ...
    print("  - Hyb laser device names: 4")
    print("  - Hyb filter device names: 4")
    print("  - Specimen ID: 1")
    print()
    print("Next steps:")
    print("  1. Obtain file paths to max projection files to extract actual acquisition times")
    print("  2. Determine probe-to-fluorophore mapping for hyb channels")
    print("  3. Fill in specimen ID")
    print()
