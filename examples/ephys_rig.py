...
digital_out0 = DAQChannel(channel_name="DO0",
                        device_name="Face Camera",
                        channel_type="Digital Output")

digital_out1 = DAQChannel(channel_name="DO1",
                        device_name="Body Camera",
                        channel_type="Digital Output")

analog_input = DAQChannel(channel_name="AI0",
                          device_name="Running Wheel",
                          channel_type="Analog Input")

harp = HarpDevice(name="Harp Behavior",
...
basestation = NeuropixelsBasestation(name='Basestation Slot 3',
...
red_laser = Laser(name='Red Laser', 
                  wavelength=473, 
                  manufacturer="Oxxius")

blue_laser = Laser(name='Blue Laser',
                   wavelength=638, 
                   manufacturer="Oxxius")

laser_module = LaserModule(manipulator=
                            Manipulator(serial_number="SN2937", 
                                        manufacturer="New Scale Technologies"),
                          lasers=[red_laser, blue_laser],
                          arc_angle=20,
                          module_angle=10)

probe_camera = Camera(name="Probe Camera",
                      data_interface="USB", 
                      manufacturer="FLIR",
                      computer_name=ephys_computer,
                      max_frame_rate=50, 
...
microscope = StickMicroscope(camera=probe_camera,
                             arc_angle=70,
                             module_angle=20)

probeA = EphysProbe(name="Probe A",
                   serial_number="9291019", 
                   probe_model="Neuropixels 1.0")

probeB = EphysProbe(name="Probe B",
                   serial_number="9291020", 
                   probe_model="Neuropixels 1.0")

ephys_moduleA = EphysModule(manipulator=Manipulator(serial_number="SN2938", 
                            manufacturer="New Scale Technologies"),
                   arc_angle=99,
                   module_angle=20,
                   rotation_angle=10,
...
ephys_moduleB = EphysModule(manipulator=Manipulator(serial_number="SN2939", 
                            manufacturer="New Scale Technologies"),
                   arc_angle=120,
                   module_angle=20,
                   rotation_angle=15,
...
filt = Filter(filter_type="Long pass", manufacturer="Thorlabs",
...
camassm1 = CameraAssembly(camera_assembly_name="Face Camera Assembly",
                        camera=face_camera,
                        filter=filt, 
                        lens=lens)
...
camassm2 = CameraAssembly(camera_assembly_name="Body Camera Assembly",
                        camera=body_camera,
                        filter=filt, 
                        lens=lens)

rig = EphysRig(rig_id="323_EPHYS1",
