...
class Metadata(DataCoreModel):
    ...
    schema_version: SkipValidation[Literal["2.0.65"]] = Field(default="2.0.65")
    name: str = Field(
        ...,
    ...
    other_identifiers: DatabaseIdentifier = Field(
        default=dict(), title="External Links", description="Links to the data asset on secondary platforms."
    )
...
def create_metadata_json(
    ...
    optional_external_identifiers: Optional[dict] = None,
) -> dict:
    ...
    if optional_external_identifiers is not None:
        params["external_identifiers"] = optional_external_identifiers
    core_fields = dict()
